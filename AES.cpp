...
void InvMixColumns(unsigned char *a){
    unsigned char b[16] ;
    b[0] = SE[a[0]] ^ SB[a[1]] ^ SD[a[2]] ^ S9[a[3]];
    b[1] = SE[a[1]] ^ SB[a[2]] ^ SD[a[3]] ^ S9[a[0]];
    b[2] = SE[a[2]] ^ SB[a[3]] ^ SD[a[0]] ^ S9[a[1]];
    b[3] = SE[a[3]] ^ SB[a[0]] ^ SD[a[1]] ^ S9[a[2]];
    b[4] = SE[a[4]] ^ SB[a[5]] ^ SD[a[6]] ^ S9[a[7]];
    b[5] = SE[a[5]] ^ SB[a[6]] ^ SD[a[7]] ^ S9[a[4]];
    b[6] = SE[a[6]] ^ SB[a[7]] ^ SD[a[4]] ^ S9[a[5]];
    b[7] = SE[a[7]] ^ SB[a[4]] ^ SD[a[5]] ^ S9[a[6]];
    b[8] = SE[a[8]] ^ SB[a[9]] ^ SD[a[10]] ^ S9[a[11]];
    b[9] = SE[a[9]] ^ SB[a[10]] ^ SD[a[11]] ^ S9[a[8]];
    b[10] = SE[a[10]] ^ SB[a[11]] ^ SD[a[8]] ^ S9[a[9]];
    b[11] = SE[a[11]] ^ SB[a[8]] ^ SD[a[9]] ^ S9[a[10]];
    b[12] = SE[a[12]] ^ SB[a[13]] ^ SD[a[14]] ^ S9[a[15]];
    b[13] = SE[a[13]] ^ SB[a[14]] ^ SD[a[15]] ^ S9[a[12]];
    b[14] = SE[a[14]] ^ SB[a[15]] ^ SD[a[12]] ^ S9[a[13]];
    b[15] = SE[a[15]] ^ SB[a[12]] ^ SD[a[13]] ^ S9[a[14]];
    for( int i = 0 ; i < 16 ; i ++ ){
        a[i] = b[i];
    }
}
...
void AES(unsigned char plaintext[16], unsigned char ciphertext[16], unsigned char k[11][16], int Rround){
    ...
    SubBytes(ciphertext);
    ShiftRows(ciphertext);
    AddRoundKey(ciphertext, k[Rround]);
}
...
void InvAES(unsigned char ciphertext[16], unsigned char plaintext[16], unsigned char k[11][16], int Rround){
    ...
    AddRoundKey(plaintext, k[Rround]);
...
int main(){
    using namespace std;
    unsigned char p[16] = {0x20, 0x14, 0xf4, 0xbc, 0xca, 0xbb, 0x6c, 0x17, 0x04, 0x4d, 0xf6, 0x61, 0x10, 0x11, 0xc0, 0x1f};
    unsigned char k[16] = {0x1d, 0x25, 0xc9, 0xf2, 0xf4, 0x17, 0xc4, 0x1b, 0xcf, 0x19, 0x02, 0x02, 0x1f, 0x1c, 0xf0, 0x11};
    unsigned char c[16];
    unsigned char rk[11][16];
    unsigned char pp[16];
    KeyExpansion(k, rk);
    cout << "The plaintext that needs to be encrypted is:\t";
    int i;
    for (i = 0; i < 16; i++){
        cout << hex << p[i];
    }
